...
namespace Google.GenAI.Tests
{
    ...
    public class InteractionsTest
    {
        ...
        public async Task TestInteractions_LegacyLyriaStreaming()
        {
            ...
            var ev1 = events[0];
            Assert.IsNotNull(ev1.Data.InteractionCreatedEvent);
            var it1 = ev1.Data.InteractionCreatedEvent!.Interaction;
            ...
            var ev2 = events[1];
            Assert.IsNotNull(ev2.Data.StepStart);
            var s2 = ev2.Data.StepStart!.Step;
        ...
        public void TestInteractions_AotAndExperimentalAttributes()
        {
            ...
            if (isNet8Assembly)
            {
                ...
                // Verify Gaos.GenAI and Gaos.IGenAI
                var genAiType = typeof(Google.GenAI.Gaos.GenAI);
                var genAiExp = genAiType.GetCustomAttribute<ExperimentalAttribute>();
                Assert.IsNotNull(genAiExp, "Gaos.GenAI should have [Experimental]");
                Assert.AreEqual("GENAI_GAOS_001", genAiExp.DiagnosticId);

                var iGenAiType = typeof(Google.GenAI.Gaos.IGenAI);
                var iGenAiExp = iGenAiType.GetCustomAttribute<ExperimentalAttribute>();
                Assert.IsNotNull(iGenAiExp, "Gaos.IGenAI should have [Experimental]");
                Assert.AreEqual("GENAI_GAOS_001", iGenAiExp.DiagnosticId);
            }
            else
            {
                ...
                var genAiType = typeof(Google.GenAI.Gaos.GenAI);
                var genAiObs = genAiType.GetCustomAttribute<ObsoleteAttribute>();
                Assert.IsNotNull(genAiObs, "Gaos.GenAI should have [Obsolete]");
                StringAssert.Contains(genAiObs.Message, "issues/159");

                var iGenAiType = typeof(Google.GenAI.Gaos.IGenAI);
                var iGenAiObs = iGenAiType.GetCustomAttribute<ObsoleteAttribute>();
                Assert.IsNotNull(iGenAiObs, "Gaos.IGenAI should have [Obsolete]");
                StringAssert.Contains(iGenAiObs.Message, "issues/159");
            }
        }
#endif
    }
