...
namespace Google.GenAI
{
  ...
  public sealed class Client : IDisposable, IAsyncDisposable
  {
        ...
        get 
        {
            if (!_warnedInteractions)
            {
                lock (_warnLock)
                {
                    if (!_warnedInteractions)
                    {
                        System.Diagnostics.Trace.TraceWarning("Warning: Interactions service is experimental and subject to change.");
    ...
    private readonly GeminiNextGenApiClient _interactionsClient;
                  ...
                  Types.ClientOptions? clientOptions = null)
    {
      ...
      _interactionsClient = new GeminiNextGenApiClient(_apiClient.InteractionsClientOptions);
      _interactions = new InteractionService(_interactionsClient);
    }
