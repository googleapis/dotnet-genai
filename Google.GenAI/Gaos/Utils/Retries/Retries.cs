...
namespace Google.GenAI.Gaos.Utils.Retries
{
    ...
    {
        private Func<int, Task<HttpResponseMessage>> action;
        ...
        public Retries(Func<int, Task<HttpResponseMessage>> action, RetryConfig retryConfig, List<string> statusCodes)
        {
            this.action = action;
        ...
        public async Task<HttpResponseMessage> Run()
        {
            switch(retryConfig.Strategy) {
                ...
                case RetryConfig.RetryStrategy.ATTEMPT_COUNT_BACKOFF:
                    return await retryWithAttemptCountBackoff(retryConfig.RetryConnectionErrors);
        ...
        private async Task<HttpResponseMessage> GetResponseAsync(bool retryConnectionErrors, int attempt)
        {
            try
            {
        ...
        private async Task<HttpResponseMessage> retryWithBackoff(bool retryConnectionErrors)
        {
            ...
            while (true)
            {
                try
                {
                    return await GetResponseAsync(retryConnectionErrors, numAttempts);
                }
        ...
        private async Task<HttpResponseMessage> retryWithAttemptCountBackoff(bool retryConnectionErrors)
        {
            var backoff = retryConfig.Backoff;
            if(backoff == null){
                throw new ArgumentException("Backoff strategy is not defined");
            ...
            var numAttempts = 0;
            var maxRetries = retryConfig.MaxRetries ?? 0;
            ...
            while (true)
            {
                ...
                catch (RetryableException ex)
                {
                    ...
                    if (intervalMs <= 0)
                    {
                        var computed = backoff.InitialIntervalMs * Math.Pow(backoff.BaseFactor, numAttempts);
                        var jitterMs = backoff.JitterFactor * computed;
                        computed = computed - new Random().NextDouble() * jitterMs;
                        computed = Math.Min(computed, backoff.MaxIntervalMs);
                        intervalMs = (long)computed;
                    }
