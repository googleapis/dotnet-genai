// Copyright 2025 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

using System.Text.Json;
using Google.GenAI.Interactions.Core;
using Google.GenAI.Interactions.Models.Interactions;

namespace Google.GenAI.Interactions.Tests.Models.Interactions;

public class SpeechConfigTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var model = new SpeechConfig
        {
            Language = "language",
            Speaker = "speaker",
            Voice = "voice",
        };

        string expectedLanguage = "language";
        string expectedSpeaker = "speaker";
        string expectedVoice = "voice";

        Assert.Equal(expectedLanguage, model.Language);
        Assert.Equal(expectedSpeaker, model.Speaker);
        Assert.Equal(expectedVoice, model.Voice);
    }

    [Fact]
    public void SerializationRoundtrip_Works()
    {
        var model = new SpeechConfig
        {
            Language = "language",
            Speaker = "speaker",
            Voice = "voice",
        };

        string json = JsonSerializer.Serialize(model, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<SpeechConfig>(
            json,
            ModelBase.SerializerOptions
        );

        Assert.Equal(model, deserialized);
    }

    [Fact]
    public void FieldRoundtripThroughSerialization_Works()
    {
        var model = new SpeechConfig
        {
            Language = "language",
            Speaker = "speaker",
            Voice = "voice",
        };

        string element = JsonSerializer.Serialize(model, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<SpeechConfig>(
            element,
            ModelBase.SerializerOptions
        );
        Assert.NotNull(deserialized);

        string expectedLanguage = "language";
        string expectedSpeaker = "speaker";
        string expectedVoice = "voice";

        Assert.Equal(expectedLanguage, deserialized.Language);
        Assert.Equal(expectedSpeaker, deserialized.Speaker);
        Assert.Equal(expectedVoice, deserialized.Voice);
    }

    [Fact]
    public void Validation_Works()
    {
        var model = new SpeechConfig
        {
            Language = "language",
            Speaker = "speaker",
            Voice = "voice",
        };

        model.Validate();
    }

    [Fact]
    public void OptionalNonNullablePropertiesUnsetAreNotSet_Works()
    {
        var model = new SpeechConfig { };

        Assert.Null(model.Language);
        Assert.False(model.RawData.ContainsKey("language"));
        Assert.Null(model.Speaker);
        Assert.False(model.RawData.ContainsKey("speaker"));
        Assert.Null(model.Voice);
        Assert.False(model.RawData.ContainsKey("voice"));
    }

    [Fact]
    public void OptionalNonNullablePropertiesUnsetValidation_Works()
    {
        var model = new SpeechConfig { };

        model.Validate();
    }

    [Fact]
    public void OptionalNonNullablePropertiesSetToNullAreNotSet_Works()
    {
        var model = new SpeechConfig
        {
            // Null should be interpreted as omitted for these properties
            Language = null,
            Speaker = null,
            Voice = null,
        };

        Assert.Null(model.Language);
        Assert.False(model.RawData.ContainsKey("language"));
        Assert.Null(model.Speaker);
        Assert.False(model.RawData.ContainsKey("speaker"));
        Assert.Null(model.Voice);
        Assert.False(model.RawData.ContainsKey("voice"));
    }

    [Fact]
    public void OptionalNonNullablePropertiesSetToNullValidation_Works()
    {
        var model = new SpeechConfig
        {
            // Null should be interpreted as omitted for these properties
            Language = null,
            Speaker = null,
            Voice = null,
        };

        model.Validate();
    }

    [Fact]
    public void CopyConstructor_Works()
    {
        var model = new SpeechConfig
        {
            Language = "language",
            Speaker = "speaker",
            Voice = "voice",
        };

        SpeechConfig copied = new(model);

        Assert.Equal(model, copied);
    }
}
