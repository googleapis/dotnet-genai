...
{
    Input = "Schedule a meeting for 10/06/2028 at 10 am with Peter and Amir about the Next Gen API",
...
foreach (var output in interaction?.Outputs ?? [])
{
    ...
    if (output.TryPickFunctionCall(out var fc))
    {
        Console.WriteLine($"Function Call: {fc.Name}");
        Console.WriteLine($"Arguments: {fc.Arguments}");
    }
}
