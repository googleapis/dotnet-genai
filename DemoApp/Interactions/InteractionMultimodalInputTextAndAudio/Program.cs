...
TextContent textContent = new() { Text = "Summarize this audio clip." };
AudioContent audioContent = new()
{
    Data = "AAAA",
    MimeType = AudioContentMimeType.AudioMp3
};

List<Content> contents = new List<Content> { Content.CreateText(textContent), Content.CreateAudio(audioContent) };

CreateModelInteraction params1 = new()
{
    Model = Model.Gemini25Flash,
    Input = InteractionsInput.CreateArrayOfContent(contents)
};
