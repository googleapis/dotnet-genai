...
CreateModelInteractionParams params1 = new()
{
    ...
    ResponseModalities = new List<ApiEnum<string, ResponseModality>> { (ApiEnum<string, ResponseModality>)ResponseModality.Image },
...
foreach (var output in interaction?.Outputs ?? [])
{
  if (output.Data != null && output.Data.Length > 100) {
    Console.WriteLine(
        $"Output: [Content with large data] Data={output.Data.Substring(0, 100)}... [truncated]");
  } else {
