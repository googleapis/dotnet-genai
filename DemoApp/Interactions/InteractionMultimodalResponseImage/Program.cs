...
{
    Model = Model.Gemini25FlashImage,
    ResponseModalities = new List<ResponseModality> { ResponseModality.Image },
    Input = InteractionsInput.CreateStr("Generate an image of a futuristic cityscape at sunset.")
};
...
if (interaction?.OutputImage?.Data != null && interaction.OutputImage.Data.Length > 100)
{
    Console.WriteLine(
        $"Output: [Content with large data] Data={interaction.OutputImage.Data.Substring(0, 100)}... [truncated]");
}
else if (interaction?.OutputImage != null)
{
    Console.WriteLine($"Output: {interaction.OutputImage.Data}");
}
