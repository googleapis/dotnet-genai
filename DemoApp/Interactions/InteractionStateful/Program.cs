...
Interaction interaction1 = await client.Interactions.Create(new() { Body = params1 });
Console.WriteLine("Model: ");
foreach (var output in interaction1?.Outputs ?? [])
{
...
Interaction interaction2 = await client.Interactions.Create(new() { Body = params2 });
Console.WriteLine("Model: ");
foreach (var output in interaction2?.Outputs ?? [])
{
    if (output.TryPickText(out var text))
    {
        Console.WriteLine(text.Text);
