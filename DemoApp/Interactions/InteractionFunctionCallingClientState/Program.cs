...
{
    Role = "user",
    Content = "Schedule a meeting for 2025-11-01 at 10 am with Peter and Amir about the Next Gen API"
});
...
Interaction response = await client.Interactions.Create(new() { Body = params1 });
...
{
    if (output.TryPickFunctionCall(out var functionCall))
    {
    ...
    else if (output.TryPickText(out var text))
    {
...
if (response?.Outputs != null)
{
    ...
    {
        Role = "model",
        Content = new TurnContent(response.Outputs)
    });
}
...
if (functionCallId != null)
{
    ...
    FunctionResultContent functionResult = new()
    {
    ...
    {
        Role = "user",
        Content = new TurnContent(new List<Content> { functionResult })
    });

    CreateModelInteractionParams followUpParams = new()
    {
        Model = Model.Gemini2_5Flash,
        Input = conversationHistory
    };

    Interaction followUpResponse = await client.Interactions.Create(new() { Body = followUpParams });

    Console.WriteLine($"Final response status: {followUpResponse.Status}");
    foreach (var output in followUpResponse?.Outputs ?? [])
    {
        if (output.TryPickText(out var text))
        {
            Console.WriteLine($"Final Output: {text.Text}");
        }
    }
}
else
